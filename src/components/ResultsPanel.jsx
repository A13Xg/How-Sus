...
function buildPDF(results, inputData) {
  ...
  if (results.type === 'image' && results.imageAnalysis) {
    line('');
    line('IMAGE ANALYSIS:', 11, true);
    const ia = results.imageAnalysis;
    if (ia.metadata && Object.keys(ia.metadata).length > 0) {
      line('  Metadata:');
      Object.entries(ia.metadata).slice(0, 6).forEach(([k, v]) => line(`    ${k}: ${v}`));
    }
    if (ia.reverseSearchMatches?.length) {
      line(`  Reverse search matches: ${ia.reverseSearchMatches.length}`);
      ia.reverseSearchMatches.slice(0, 3).forEach((m) => line(`    ${m.source || m.url || 'Unknown source'}`));
    }
  }

  // Source freshness
  const freshnessFinding = (results.findings || []).find((f) => f.label === 'Source freshness');
  if (freshnessFinding) {
    line('');
    line('SOURCE FRESHNESS:', 11, true);
    line(`  ${freshnessFinding.value}`);
    if (freshnessFinding.excerpt) line(`  ${freshnessFinding.excerpt}`);
    (freshnessFinding.dataPath || []).forEach((p) => line(`    ${p}`));
  }

  // Claim density
  const claimFinding = (results.findings || []).find((f) => f.label === 'Claim density');
  if (claimFinding) {
    line('');
    line('CLAIM DENSITY:', 11, true);
    line(`  ${claimFinding.value}`);
    if (claimFinding.excerpt) line(`  ${claimFinding.excerpt}`);
  }

  // Data path trail for top 3 findings with dataPath
  const trailFindings = (results.findings || []).filter((f) => f.dataPath?.length).slice(0, 3);
  if (trailFindings.length > 0) {
    line('');
    line('DATA PATH TRAIL (top findings):', 11, true);
    trailFindings.forEach((f) => {
      line(`  ${f.label}:`);
      f.dataPath.forEach((p) => line(`    • ${p}`));
    });
  }
            ...
            {confidenceScore > 0 && (
              ...
              >
                ...
                <span className="confidence-detail">
                  {confidenceScore >= 65 ? ' · Strong evidence' : confidenceScore >= 35 ? ' · Moderate evidence' : ' · Limited evidence'}
                </span>
              </span>
            )}
          </div>
